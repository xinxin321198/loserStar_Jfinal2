/**
 * 基于bootstrap3的模态对话框封装的树形数据选择数据的弹出窗口，使用原型链的方式封装，此组件树结构依赖于jQuery treetable https://plugins.jquery.com/treetable
 * 
用法：
    //构建查询条件配置项
    //filedName显示的字段名，
    ...
    var fileOpt = {
        ...
        //配置数据的主键字段
        querSelectDataPrimaryKey:"id",
        //指定用于构建树的父节点字段
        querSelectDataParentKey:'pid'
        //配置操作列的按钮（
        id：按钮的id标识，
        name：按钮显示的名称、
    ...
    addMainDataWindow.getSelectedRow();//返回选中的行的dom对象
    addMainDataWindow.expandAll();//展开全部树节点
    addMainDataWindow.collapseAll();//收缩全部树节点
*/
 ...
 * @returns 
 */
var loserStarTreeDataSelectBootstrapWindow = function (opt) {
    this.init(opt);
}

loserStarTreeDataSelectBootstrapWindow.prototype = {
    constructor: loserStarTreeDataSelectBootstrapWindow,
    init: function (opt) {
        ...
        this.querSelectDataParentKey = opt.querSelectDataParentKey;//指定用于构建树的父节点字段
        ...
        this.selectedCallback = opt.selectedCallback;//选中一行的回调方法，会传入选中的行的dom对象
        //执行一些初始化的方法
    ...
    createElement: function () {
        var self = this;
        var text = "";
        text += "<!-- loserStarTreeDataSelectBootstrapWindow--begin -->";
        ...
        text += "                       <div id=\"toolbar\" class=\"btn-group\">";
        text += "                        <button id=\"" + self.flagId + "_queryBtn\" type=\"button\" class=\"btn bg-olive\"><span class=\"fa fa-search\"></span>搜索</button>";
        text += "                            <button id=\"" + self.flagId +"_expandAllBtn\" type=\"button\" class=\"btn bg-olive\" >";
        text += "                                <span class=\"fa fa-angle-double-down\"></span>全部展开";
        text += "                            </button>";
        text += "                            <button id=\"" + self.flagId +"_collapseAllBtn\" type=\"button\" class=\"btn bg-olive\">";
        text += "                                <span class=\"fa fa-angle-double-up\"></span>全部收缩";
        text += "                            </button>";
        text += "                        </div>";

        text += "                        <table id=\"" + self.flagId + "_treeDataTable\" class=\"table table-bordered table-condensed\">";
        text += "                            <thead>";
        text += "                                <tr>";
        if(self.isShowIndex){
            text += "                                    <th>#</th>";
        ...
        text += "        </div>";
        text += "        <!-- /.modal -->";
        text += "        <!-- loserStarTreeDataSelectBootstrapWindow--end -->";
        ...
        //绑定全部展开按钮的事件
        $("#" + self.flagId + "_expandAllBtn").on("click", function () {
            self.expandAll();
        });
        //绑定全部收缩按钮的事件
        $("#" + self.flagId + "_collapseAllBtn").on("click", function () {
            self.collapseAll();
        });
    },
    ...
    renfreshTableData: function (list, disabledIdList) {
        ...
        for (var i = 0; i < self.list.length; i++) {
            var tmp = self.list[i];
            text += "                                <tr id=\"" + tmp[self.querSelectDataPrimaryKey] + "\" data-id=\"" + tmp[self.querSelectDataPrimaryKey] + "\" data-pid=\"" + tmp[self.querSelectDataParentKey] +"\">";
            if(self.isShowIndex){
                text += "                                    <td>" + (i + 1) + "</td>";
            }
            for (j = 0; j < self.displayTableFiled.length; j++) {
    ...
    //初始化tree结构，基于jQuery treetable https://plugins.jquery.com/treetable
    initTreeTable:function(){
        var self = this;
        var table = $("#" + self.flagId +"_treeDataTable").treetable({
            //树是否可以展开收缩
            expandable: true,
            //点击节点名称也可以展开
    ...
    getSelectedRow:function(){
        var self = this;
        return $("#" + self.flagId + "_treeDataTable tr.selected");
    },
    //展开全部
    expandAll:function(){
        var self = this;
        $("#" + self.flagId +"_treeDataTable").treetable("expandAll");
    },
    //收缩全部
    collapseAll:function(){
        var self = this;
        $("#" + self.flagId + "_treeDataTable").treetable("collapseAll");
    }
