...
listPageEvent.queryList = function () {
    //构建查询条件
    var queryObj = {};
    queryObj.id = $("#id").val();//主键id
    queryObj.name = $("#name").val();//名称
    queryObj.path = $("#path").val();//文件存储路径
    queryObj.upload_time = $("#upload_time").val();//上传日期
    queryObj.sort = $("#sort").val();//排序
    queryObj.del = $("#del").val();//软删除字段
    ...
    queryObj.suffix = $("#suffix").val();//文件后缀
    queryObj.from_id = $("#from_id").val();//附件所属记录id
    queryObj.from_table = $("#from_table").val();//附件所属记录的表
    sysFileAction.getListData(queryObj, function (data) {
        //绘制表头和表尾
        var thead = "";
        thead += "                            <tr>";
        ...
        thead += "                                <th>更新人姓名</th>";
        thead += "                                <th>文件后缀</th>";
        thead += "                                <th>附件所属记录id</th>";
        thead += "                                <th>附件所属记录的表</th>";
        thead += "                                <th>操作</th>";
        thead += "                            </tr>";
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            text += "                            <td id=\"" + tmp.id + "_name\">" + tmp.name + "</td>";
            text += "                            <td id=\"" + tmp.id + "_path\">" + tmp.path + "</td>";
            text += "                            <td id=\"" + tmp.id + "_upload_time\">" + tmp.upload_time + "</td>";
            text += "                            <td id=\"" + tmp.id + "_sort\">" + tmp.sort + "</td>";
            ...
            text += "                            <td id=\"" + tmp.id + "_suffix\">" + tmp.suffix + "</td>";
            text += "                            <td id=\"" + tmp.id + "_from_id\">" + tmp.from_id + "</td>";
            text += "                            <td id=\"" + tmp.id + "_from_table\">" + tmp.from_table + "</td>";
            text += "                            <td>";
        ...
        //最后初始化datatables
        table = loserStarDatatablesCfg.initDataTables("sys_file_table");
    });
}
...
listPageEvent.queryPageList = function () {
    ...
    queryObj.path = $("#path").val();
    queryObj.upload_time = $("#upload_time").val();
    queryObj.sort = $("#sort").val();
    ...
    queryObj.suffix = $("#suffix").val();
    queryObj.from_id = $("#from_id").val();
    queryObj.from_table = $("#from_table").val();
    //分页参数
    ...
    sysFileAction.getPage(queryObj, function (data) {
        console.log(data);
